...
def jione_truthfulqa_body(model, task, batch):
    from src.prediction import phi_qwen_prediction, predict_batch
    ...
    prediction = ""
    messages = None
    if model.get('task') == "generation":
        prediction = detect_choice(phi_qwen_prediction(mod, tok, batch, task="truthful_qa"), choices)
    else:
        prediction = predict_batch(input_text, mod)
    ...
    if task == "generation":
        ...
        messages = prompt
        # print("distill is teacher here is the input", prompt)
        
    return messages


def jione_mbpp_body(model, task, batch):
    from src.prediction import phi_qwen_prediction, predict_batch
    input_text = batch['input'][0].strip()
    mod = model.get("mod")
    tok = model.get("tok")
    ...
    messages = None
    if model.get('task') == "generation":
        prediction = extract_before_system(phi_qwen_prediction(mod, tok, batch, task="mbpp"))
    else:
        prediction = predict_batch(input_text, mod)
...
def jione_gsm8k_body(model, task, batch):
    from src.prediction import phi_qwen_prediction, predict_batch
    input_text = batch['input'][0].strip()
    ...
    messages = None
    if model.get('task') == "generation":
        prediction = extract_final_answer(phi_qwen_prediction(mod, tok, batch, task="gsm8k"))
    else:
        prediction = predict_batch(input_text, mod)
    ...
    2. If the solution is not correct or equal to 'None', solve the problem and give your solution precedding by #### at the end of the resolution
    """
...
def jione_sst2_body(model, task, batch):
    from src.prediction import phi_qwen_prediction, predict_batch
    ...
    messages = None
    if model.get('task') == "generation":
        student_prediction = extract_sentiment(phi_qwen_prediction(mod, tok, batch))
    else:
        student_prediction = predict_batch(text, mod)
